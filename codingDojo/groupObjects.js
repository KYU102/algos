...
function groupObjects(items) { 
    ...
    console.log(Object.keys(returnObj)[2])
    for(let i = 0; i < items.length; i++){
        if (items[i].category.toLowerCase() === "cute"){
            returnObj.cute.push(items[i])
        }
        if (items[i].category.toLowerCase() === "food"){
            returnObj.food.push(items[i])
        }
        if (items[i].category.toLowerCase() === "cradle of civilization"){
            returnObj.['Cradle of Civilization'].push(items[i])
        }
    }
    return returnObj
}
